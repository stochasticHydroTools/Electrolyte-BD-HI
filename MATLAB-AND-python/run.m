...
gridfac=1.2; % grid spacing = g_t/gridfac (upsampling in GPU)
...
if (~pargiven)
    nxy = 72;% choose grid size (Ewald param calculated from that)
end
...
numSds = 4; % consistent with the GPU code
Hew = 1.25*numSds*gtot; % 1/2 the distance where we include images
...
[phihin, Exhin, Eyhin, Ezhin] = solveDPEwald(gf_C1+gf_C2+gf_C1star,nxy,Nz,Hs,thet0,kvec,wallallimg);
